...
int fcComd(int);               //比较文件
int moveComd(int);             //文件搬家
int batchComd(int);             //批处理
int replaceComd(int);           //文件取代
...
int main()
{
    ...
    // 读入文件分配表FAT
    char yn;
    ifstream ffi("C:\\Users\\youshuaiHou\\CLionProjects\\untitled\\FAT2008.txt");//打开文件FAT2008.txt
    if (!ffi)
    {
    ...
    //读入磁盘块Disk[ ]信息
    ffi.open("C:\\Users\\youshuaiHou\\CLionProjects\\untitled\\Disk2008.dat",ios::binary);
    if (!ffi)
    {
    ...
    ffi.open("C:\\Users\\youshuaiHou\\CLionProjects\\untitled\\UdTab2008.dat",ios::binary);
    if (!ffi)
    {
...
void ExecComd(int k)		//执行命令
{
    ...
    char CmdTab[][COMMAND_LEN]={"create","open","write","read","close",
                                ...
                                "undel","Prompt","udtab","fc","move","batch","replace"};
    int M=sizeof(CmdTab)/COMMAND_LEN;	//统计命令个数
    for (cid=0;cid<M;cid++)			//在命令表中检索命令
        if (strcmp(CmdTab[cid],comd[0])==0)//命令不区分大小写
            break;
    ...
    switch(cid)
    {
        ...
        case 26:fcComd(k);         //fc
            break;
        case 27:moveComd(k);       //move
            break;
        case 28:batchComd(k);       //batch
            break;
...
//必须查看了才能退出，修改为输入“-1”退出help
//查看完一条帮助后不会立即退出，可同时查看其他所需帮助，不用每次都输入help进入帮助查看

{
    cout << "\n* * * * * * * * * 本系统主要的文件操作命令简述如下，输入序号可查看命令详情 * * * * * * * * * *\n\n";
    cout << "1：create <文件名>[ <文件属性>]\n";
    cout << "2：open <文件名>\n";
    cout << "3：write <文件名> [<位置/app>[ insert]]\n";
    cout << "4：read <文件名> [<位置m> [<字节数n>]]\n";
    cout << "5：close <文件名>\n";
    cout << "6：del <文件名>\n";
    cout << "7：dir [<路径名>] [|<属性>]\n";
    cout << "8：cd [<路径名>]\n";
    cout << "9：md <目录名>[ <属性>]\n";
    cout << "10：rd [<路径名>]\n";
    cout << "11：ren <旧文件名> <新文件名>\n";
    cout << "12：attrib <文件名> [±<属性>]\n";
    cout << "13：copy <源文件名> [<目标文件名>]\n";
    cout << "14：type <文件名>\n";
    cout << "15：rewind <文件名>\n";
    cout << "16：fseek <文件名> <位置>\n";
    cout << "17：block <文件名>\n";
    cout << "18：closeall\n";
    cout << "19：uof\n";
    cout << "20：undel [<路径名>]\n";
    ...
    cout << "23：fat\n";
    cout << "24：check\n";
    cout << "25:fc <文件名1> <文件名2>\n";
    cout << "26:move <文件名> <目录名>\n";
    cout << "27:batch batchfile\n";
    cout << "28:replace <文件名> <目录名>\n";
    cout << "输入“-1”退出帮助\n";
    ...
    switch (number) {
        //内容来自文档2.3操作系统命令简介
        case 1:
            cout << "创建文件：create <文件名> [<属性>]。创建一个指定名字的新文件，即在目录中增加一目录项，不考虑文件的内容。对于重名文件给出错误信息。";
            break;
        case 2:
            cout << "打开文件：open <文件名>。若指定文件存在且尚未打开，则打开之，并在用户打开文件表（UOF）中登记该文件的有关信息。若指定文件已经打开，则显示“文件已打开”的信息；若指定文件不存在，则给出错误信息。只读文件打开后只能读不能写。\n";
            break;
        case 3:
            cout << "写文件：write <文件名> [<位置>][insert]。在文件的指定位置处写入新内容。\n";
            break;
        case 4:
            cout << "读文件：read <文件名> [<位置m> [<字节数n>]]。从已打开文件中读指定内容并显示。\n";
            break;
        case 5:
            cout << "关闭文件：close <文件名>。若指定文件已打开，则关闭之，即从UOF中删除该文件对应的表项。若文件未打开或文件不存在，分别给出有关信息。\n";
            break;
        case 6:
            cout << "删除文件：del <文件名>。删除指定的文件，即清除其目录项和回收其所占用磁盘空间。对于只读文件，删除前应询问用户，得到同意后方能删除。当指定文件正在使用时，显示“文件正在使用，不能删除”的信息，当指定文件不存在时给出错误信息。\n";
            break;
        case 7:
            cout << "显示目录：dir[ <目录名>[ <属性>]]。显示“目录名”指定的目录中文件名和第一级子目录名。若指定目录不存在，则给出错误信息。\n";
            break;
        case 8:
            cout << "改变当前目录：cd [<目录名>]。当前目录（工作目录）转移到指定目录下。指定目录不存在时，给出错误信息。若命令中无目录名，则显示当前目录路径。\n";
            break;
        case 9:
            cout << "创建目录：md <目录名>。在指定路径下创建指定目录，若没有指定路径，则在当前目录下创建指定目录。对于重名目录给出错误信息。\n";
            break;
        case 10:
            cout << "删除目录：rd 目录名。若指定目录为空，则删除之，否则，给出“非空目录不能删除”的提示。不能删除当前目录。\n";
            break;
        case 11:
            cout << "文件改名：ren <原文件名> <新文件名>。将指定文件的名字改为“新文件名”。若原文件不存在，给出错误信息。若原文件存在，但正在使用，也不能改名，同样显示出错信息。应检查新文件名是否符合命名规则以及是否存在重名问题。\n";
            break;
        case 12:
            cout << "显示、修改文件或目录属性：attrib <文件名>[ <文件属性>]。若命令中无“文件属性”参数，则显示指定文件的属性；若命令中有“文件属性”参数，则修改指定文件的属性。此命令对目录名同样适用。\n";
            break;
        case 13:
            cout << "复制文件：copy <源文件名> <目标文件名>。命令功能：为目标文件建立目录项，分配新的盘块，并将源文件的内容复制到目标文件中。\n";
            break;
        case 14:
            cout << "显示文件内容：type <文件名>。显示指定文件的内容。若指定文件不存在，则给出错误信息。\n";
            break;
        case 15:
            cout << "rewind <文件名>。将指定文件的读、写指针移到文件开头(第一个字节处)。\n";
            break;
        case 16:
            cout << "fseek <文件名> <n>。将读、写指针移到指定位置n处。\n";
            break;
        case 17:
            cout << "block <文件名>。显示文件或目录占用的磁盘块号。若指定文件或目录不存在则报错。\n";
            break;
        case 18:
            cout << "关闭当前打开的所有文件。\n";
            break;
        ...
        case 22:
            ...
            break;
        case 23:
            cout << "显示FAT表中空闲盘块数(0的个数)。\n";
            break;
        case 24:
            cout << "核对后显示FAT表中空闲盘块数。\n";
            break;
        case 25:
            cout << "逐个字节比较指定的两个文件，若相同，显示“文件内容相同”字样；若不同，显示第一个不同字节的位置和各自的内容。若文件不存在则报错。\n";
            break;
        case 26:
            cout << "将“文件名”指定的文件或目录，移动到“目标名”指定目录中(名字保持不变)。此命令也可改变子目录名。“文件名”指定被操作的文件或目录，可以是目录名；\n“目录名”指定“文件名”的新位置或子目录的新名字，即必须是目录名，不能是文件名。文件或子目录转移时不能改名。\n";
            break;
        case 27:
            cout << "从磁盘文件batchfile逐行读入命令行执行，直到batchfile中所有命令行都执行完毕为止。\n";
            break;
        case 28:
            cout << "以“文件名”指定的文件，取代“目录名”指定目录中的同名文件。\n";
            break;
        case -1:
            i = 0;
            cout<<"已退出help。";
...
/***************************在这里我是按简单点的来直接注释掉了这段，然后将下面的for(i=1 改为
 * for（i=0 ，否则会不读第一个属性字符。
 * 但是如果时间够的话，想在attrib 变量前加一个“|”
    if (str[0]!='|')
    {
        cout<<"\n命令中属性参数错误。\n";
        return -1;
...
////////////////////////////////修改
    ...
    {
        if(uof[0].state==0)
            cout<<"\n文件"<<temppath<<"当前无操作文件，请输入要显示的文件名。\n";
        else{
            for (i_uof=1;i_uof<(S+1);i_uof++)
            {
...
#define BSIZE 40*SIZE+1
    ...
    {
        if (strncmp(comd[2],"app",3)==0)
            pos=uof[ii_uof].fsize+1;	//文件尾部添加模式(write <文件名> append)
        else if (strncmp(comd[2],"ins",3)==0)
        {
...
int CopyComd(int k)		//copy命令的处理函数：复制文件
{
    ...
    else	//k=2(命令中提供目标文件)的情况
    {
        ///////////////////////////////修改///////////
        if(!strcmp(comd[2],"..")){
            int s= FindPath(comd[2],(char)16,1,fcbp);//找指定目录的首块号
            if(s<1){
                cout<<"路径错误！\n";
                return -1;
            }
...
int fcComd(int k) {
    ...
    s01 = ProcessPath(comd[1], FileName1, k, 0, '\20');//取文件1所在目录的首块号
    if (s01 < 1)                                    //路径错误
    {
        return s01;                                //失败返回
    }

    s1 = FindFCB(FileName1, s01, attrib, fcbp);        //取文件1首块号
    fcbp1 = fcbp;
    strcpy(gFileName1, temppath);
    i = strlen(temppath);
    if (temppath[i - 1] = '/')
        strcat(gFileName1, "/");
    strcat(gFileName1, FileName1);                //构造文件全路径名

/**********************************************************************************/
    s02 = ProcessPath(comd[2], FileName2, k, 0, '\20');//取文件2所在目录首块号
    if (s02 < 1) {
        return s02;
    }
    s2 = FindFCB(FileName2, s02, '\40', fcbp);        //取文件2首块号
    fcbp2 = fcbp;
    strcpy(gFileName2, temppath);
			i=strlen(temppath);
			if(temppath[i-1]='/')
			strcat(gFileName2,"/");
    strcat(gFileName2, FileName2);                //构造文件全路径名
    ...
    if(s1==0||s2==0) {
        if (s1 == 0) {
            cout << "文件" << gFileName1 << "是空文件";
            return -1;
        }
        if (s2 == 0) {
            cout << "文件" << gFileName2 << "是空文件";
            return -1;
        }
    }
    //判断输入参数是否为同一个值
    if (s1 == s2 && s1 != 0 && s2 != 0) {
        cout << "请勿输入同一个文件进行比较" << endl;
        return -3;
    }
        //输入参数正确，开始进行比较
    else {
        size1 = fcbp1->Fsize;                //文件大小
        size2 = fcbp2->Fsize;
        ...
        while (s1 > 0 && s2 > 0) {
            ...
            for (i = 0; i < SIZE; i++, j++) {
                if (j == size1 + 1 || j == size2 + 1)    // 判断buffer1或buffer2是否已经遍历完
                    break;
                Buffer1[j] = Disk[s1][i];
                Buffer2[j] = Disk[s2][i];
                ...
                if (Buffer1[j] != Buffer2[j]) {
                    ...
                    cout << "文件" << gFileName1 << "长度为" << size1 << endl
                         << "文件" << gFileName2 << "长度为" << size2 << endl
                         << "第一个不同的字节为第" << j + 1 << "个字节" << endl
                         << "此时文件" << gFileName1 << "和文件" << gFileName2 << "不同的内容是：" << endl
                         << gFileName1 << "  " << Buffer1[j] << endl
                         << gFileName2 << "  " << Buffer2[j] << endl;
        ...
        if ((j - 1) == size1 && (j - 1) == size2) {
            cout << "文件相同，且文件长度为" << size1;
            return 1;
        }
...
int batchComd(int k)
{
    ...
    s=FindFCB(FileName,s,attrib,fcbp);					//取FileName文件首块号
    strcpy(gFileName,temppath);
    i=strlen(temppath);
    if(temppath[i-1]!='/')
        strcat(gFileName,"/");
    ...
    {
        size=fcbp->Fsize;
        char Buffer[size+1][size+1];								//分配动态内存空间
...
//    if (!IsName(FileName2))		//若名字不符合规则
  //  {
    ...
    *fcbp2=*fcbp1;						//源文件的目录项复制给目标文件
    ...
    swap(comd[1],comd[2]);//交换值
    CdComd(1);
    swap(comd[1],comd[2]);
    s01=ProcessPath(comd[1],FileName1,k,0,'\20');//取FileName所在目录的首块号
    FindFCB(comd[1],s01,attrib,fcbp3);
    if((int)fcbp3->Fattrib == 2 || (int)fcbp3->Fattrib == 3 ||
       (int)fcbp3->Fattrib == 4 || (int)fcbp3->Fattrib == 5 ||
       (int)fcbp3->Fattrib == 6 || (int)fcbp3->Fattrib == 7)
    {
        cout<<"\n具有隐藏和系统属性的文件不能被取代。\n";
        return -7;
    }
